...
test "source-less surface paint keeps transcript lines one to one under full transcript" {
    var sink = try std.Io.Dir.openFileAbsolute(io_mod.getIo(), io_mod.null_device, .{ .mode = .write_only });
    defer sink.close(io_mod.getIo());
...
test "streamed chunk-split produces same viewport as single emission" {
    ...
    var sink1 = try std.Io.Dir.openFileAbsolute(io_mod.getIo(), io_mod.null_device, .{ .mode = .write_only });
    defer sink1.close(io_mod.getIo());
    var sink2 = try std.Io.Dir.openFileAbsolute(io_mod.getIo(), io_mod.null_device, .{ .mode = .write_only });
