...
fn ensureManagedDir(parent_path: []const u8, child_name: []const u8) !void {
    ...
    for (0..2) |_| {
        ...
        io_mod.setDirPermissions(child, zio, io_mod.permissionsFromMode(0o700)) catch
            return error.CorruptArtifactStore;
        const stat = try child.stat(zio);
        if (stat.kind != .directory or !io_mod.hasMode(stat.permissions, 0o700)) {
            return error.CorruptArtifactStore;
        }
        return;
    }
...
test "web_fetch artifact store creates private managed directories" {
    ...
    try std.testing.expectEqual(@as(io_mod.Mode, 0o700), io_mod.permissionsModeOrZero(artifacts_stat.permissions));
    try std.testing.expectEqual(@as(io_mod.Mode, 0o700), io_mod.permissionsModeOrZero(web_fetch_stat.permissions));
}
