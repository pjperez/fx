...
fn normalizeAndVerifyPrivateDir(dir: std.Io.Dir) !void {
    ...
    if (!io_mod.isPermissionWritable(initial.permissions)) {
...
test "credential store is private atomic and supports restart deletion" {
    ...
    try std.testing.expectEqual(
        @as(u32, 0o700),
        io_mod.permissionsModeOrZero(root_stat.permissions),
    );
    ...
    try std.testing.expectEqual(
        @as(u32, 0o700),
        io_mod.permissionsModeOrZero(dir_stat.permissions),
    );
    ...
    try std.testing.expectEqual(
        @as(u32, 0o600),
        io_mod.permissionsModeOrZero(file_stat.permissions),
    );
